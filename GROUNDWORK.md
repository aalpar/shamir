...
Need to formalize what "security" means for a threshold secret sharing
system before making protocol choices. Everything else depends on this:
...
- [ ] **Herzberg et al. (1995)** — already read for `pkg/refresh/`, but
  ...
  *Why:* The original proactive security model assumes a mobile
  adversary that can corrupt up to t servers per epoch. Need to define
  what "corrupted server" means for this library's deployment contexts.

### Decision this unblocks

What are the actual security guarantees? "An attacker who compromises
fewer than k shareholders within a single refresh interval learns nothing
about the secret" — is that the claim? What about a compromised
coordinator?
...
settings (Cachin 2002) only matters once we know our threat model. An
...
- [ ] **Cachin et al. (2002)** — "Asynchronous Verifiable Secret Sharing
  ...
  *Why:* First async VSS + proactive refresh. Identifies the
  privacy-correctness trade-off. Need to understand whether bounded-delay
  environments sidestep the impossibility result.
...
- [ ] **Zhou, Schneider, van Renesse (2005)** — "APSS: Proactive Secret
  ...
  *Why:* Full async PSS protocol. Models async message delays directly.
  Need to extract the protocol structure for the coordination layer.
...
- [ ] **Günther, Das, Kokoris-Kogias (2022)** — "Practical Asynchronous
  ...
  *Why:* Practical constructions at O(n³) and O(cn²). Need to evaluate
  communication patterns (centralized relay vs. peer-to-peer).
...
- [ ] **Yurek et al. (2023)** — "Robust Asynchronous DPSS"
  ...
  *Why:* Shows synchronous DPSS degrades O(n³)→O(n⁴) during async
  periods. Directly argues against a synchronous model. Also handles
  dynamic committees — relevant if shareholders can join and leave.
...
Should refresh use synchronous epochs (current design:
`Active → Refreshing → Active`) or async contribution-based refresh
(CRDT-style: holders apply verified deltas as they arrive, coordinator
tracks progress)?
...
informs how the coordination layer tracks refresh progress and when old
...
How does the coordination layer detect "all holders have applied all
contributions for epoch E" (the stable cut / checkpoint) without requiring
synchronous coordination? What metadata does each shareholder need to track?

---

## 4. Distributed Key Generation (DKG)

A key ceremony where no single party sees the plaintext secret. Already
in BIBLIOGRAPHY.md but needs deeper study.

Depends on: **Threat Model** — whether the coordinator is trusted determines
...
- [ ] **Gennaro et al. (1999)** — "Secure Distributed Key Generation for
  ...
  *Why:* A coordinator generating and splitting the secret defeats
...
Does initial secret splitting require a trusted dealer (coordinator sees
plaintext), or can we do DKG where the coordinator orchestrates without
seeing the secret?
...
If shareholders can join and leave, the refresh protocol needs to handle
committee changes.
...
- [ ] **Yurek et al. (2023)** — same paper as above, but focus on the
  ...
  *Why:* Any multi-party system will face shareholder churn. Need
